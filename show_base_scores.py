...
# # --- Interactive Dot Adjustment ---
# st.subheader("They can be fine-tuned")
# # --- Inject JavaScript/HTML ---
...
# let dots = {json.dumps(initial_positions)};
...
# // Draw function using 1 - x positions
# function draw() {{
...
#     // Dots
#     dots.forEach((pos, i) => {{
#         let flippedX = (1 - pos) * canvas.width;
#         ctx.beginPath();
#         ctx.arc(flippedX, 50, 8, 0, 2 * Math.PI);
...
#         ctx.fillText(labels[i], flippedX, 30);
#     }});
# }}
...
#     draggingIndex = dots.findIndex(pos => {{
#         let flippedX = (1 - pos) * canvas.width;
#         return Math.abs(flippedX - x) < 10;
#     }});
# }});
...
#         let rect = canvas.getBoundingClientRect();
#         let x = e.clientX - rect.left;
#         let flippedPos = 1 - (x / canvas.width);
#         dots[draggingIndex] = Math.min(1, Math.max(0, flippedPos));
#         draw();
#     }}
# }});
# </script>
...
# # Create a slider for each dot
# st.subheader("Adjust dot positions")
# dot_positions = []
# for i, label in enumerate(labels):
#     pos = st.slider(f"Position of '{label}'", 0.0, 1.0, initial_positions[i], key=f"slider_{i}")
...
#     yaxis=dict(range=[-0.2, 0.5], showgrid=False, zeroline=False, visible=False),
#     height=200,
